...
import { importarAbastecimentos, type ResultadoImportacao } from "../actions";
...
export function ImportForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const resposta = await importarAbastecimentos(undefined, formData);
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="card space-y-4 p-6">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Baixe o modelo acima, preencha uma linha por abastecimento e envie o arquivo aqui.
          </p>
        </div>
        <button type="submit" disabled={isPending} className="btn-primary disabled:opacity-50">
          {isPending ? "Importando..." : "Importar abastecimentos"}
        </button>
      </form>
