...
export function HistoricoInspecoes({ placa, inspecoes }: { placa: string; inspecoes: Inspecao[] }) {
  ...
  return (
    <div className="space-y-6">
            ...
            {pendencias.map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-3 rounded-lg bg-red-50 px-3 py-2 text-sm">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="shrink-0 rounded-md bg-white dark:bg-slate-800 px-2.5 py-1 text-xs font-medium text-red-700 shadow-sm hover:bg-red-100"
                >
                  Marcar resolvida
                </button>
              </div>
      ...
      <div>
        <h3 className="mb-2 text-sm font-semibold text-slate-900 dark:text-slate-100">Histórico de inspeções</h3>
        <div className="space-y-3">
          {inspecoes.map((insp) => {
            const naoConformes = insp.itens.filter((it) => !it.conforme);
            return (
              <details key={insp.id} className="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                <summary className="flex cursor-pointer items-center justify-between gap-2 text-sm">
                  ...
                  <span className="flex items-center gap-2">
                    {insp.origem === "motorista" ? (
                      ...
                      <span className="rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-xs text-slate-600 dark:text-slate-300">Gestor</span>
                    )}
