...
export async function importarPostosGf(
  _prev: ResultadoImportacaoPostosGf | undefined,
  formData: FormData
): Promise<ResultadoImportacaoPostosGf> {
  // A gravação em si roda com o client admin (ver mais abaixo), então essa
  // checagem de sessão é a única barreira contra chamar a action deslogado.
  ...
  if (!empresaId) return { erro: "Selecione o cliente dono desta rede de postos." };

  const arquivo = formData.get("arquivo");
  if (!(arquivo instanceof File) || arquivo.size === 0) {
    return { erro: "Selecione o arquivo postos_gf.xlsx." };
  }
  ...
  return { total: linhas.length - 1, sucesso, erros, duplicadas, conflitantes };
}
