...
import {
  prepararImportacaoVeiculos,
  processarLoteVeiculos,
  type LinhaResultado,
} from "../actions";
import { calcularPaginacao } from "@/components/Paginacao";

// Fase Indicador-Progresso-Import (12/08/2026, pedido do Daniel: planilhas
// de milhares de linhas sem nenhum indicador de andamento na tela) -- a
// importação agora acontece em duas etapas (ver actions.ts): primeiro
// prepararImportacaoVeiculos (lê e decide o que fazer com cada linha, sem
// gravar nada) e depois processarLoteVeiculos, chamada repetidamente daqui
// -- um lote de TAMANHO_LOTE linhas de cada vez -- atualizando a barra de
// progresso entre uma chamada e outra. Uma Server Action só não dá pra
// "espiar" de fora enquanto roda; por isso o trabalho pesado foi dividido
// em várias chamadas menores.
...
export function ImportForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      const prep = await prepararImportacaoVeiculos(formData);
      if ("erro" in prep) {
        setErro(prep.erro);
        setProgresso(null);
        return;
      ...
      for (let i = 0; i < prep.preparadas.length; i += TAMANHO_LOTE) {
        ...
        const resultadosLote = await processarLoteVeiculos(lote);
      ...
      >
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Baixe o modelo acima, preencha uma linha por veículo e envie o
