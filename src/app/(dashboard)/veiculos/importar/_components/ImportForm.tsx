...
import { importarVeiculos, type ResultadoImportacao } from "../actions";
import { calcularPaginacao } from "@/components/Paginacao";

// Fase Corrige-Timeout-Import-Grande (12/08/2026, pedido do Daniel: cliente
// com mais de 2500 veículos na base) -- pagina a tabela de resultado em vez
// de renderizar tudo de uma vez. Reaproveita `calcularPaginacao` (o mesmo
// helper usado em /veiculos, /motoristas, /abastecimentos etc. -- já
// comprovado em telas com milhares de registros) em vez de inventar uma
// lógica de paginação nova só pra esta tela. A UI abaixo é uma versão
// client-side do componente `Paginacao` (que é Server Component e navega
// por Link/URL): aqui não há URL própria pro resultado de um import
// (é estado local, populado por uma Server Action), então os cliques
// trocam `pagina` em vez de navegar.
const POR_PAGINA = 50;
...
export function ImportForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const resposta = await importarVeiculos(undefined, formData);
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="card space-y-4 p-6">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Baixe o modelo acima, preencha uma linha por veículo e envie o arquivo aqui.
          </p>
        </div>
        <button type="submit" disabled={isPending} className="btn-primary disabled:opacity-50">
          {isPending ? "Importando..." : "Importar veículos"}
        </button>
      </form>
