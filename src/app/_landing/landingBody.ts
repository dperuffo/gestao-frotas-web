...
/* Fase Design-System-Soft-UI-Evolution (27/08/2026, pedido do Daniel: "a
   landing page da aplicação será neste design" — 3º design.md, escopo
   restrito à landing por pedido explícito dele — dashboard/menu/auth
   continuam no tema Swiss Minimalism aplicado antes). Paleta pastel: Soft
   Blue (#87CEEB, acento principal — usado em quase tudo: CTAs, badges, UI
   do mockup do hero), Soft Pink (#FFB6C1) e Soft Green (#90EE90) como
   toques secundários. O tema não define nenhuma cor de fundo escuro
   própria — mantive a "casca" dark do hero (ele já é assim estruturalmente
   nas 915 linhas deste arquivo, com texto branco/cinza-claro sobre fundo
   escuro; reescrever pra fundo claro seria um rebuild completo, não um
   reskin) só que num tom mais suave (slate escuro, não navy puro),
   coerente com "Light/Dark: Full/Full" do próprio design.md. */
:root{--navy:#26303d;--blue:#33404e;--electric:#ffb6c1;--cyan:#87ceeb;--cyan2:#a8def0;--gold:#90ee90;--white:#ffffff;--gray:#a9b4c0;}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
html{scroll-behavior:smooth;}
body{font-family:'Inter',sans-serif;background:var(--navy);color:var(--white);overflow-x:hidden;}
nav{position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:16px 6%;background:rgba(38,48,61,0.92);backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,0.06);}
...
.hero-bg{position:absolute;inset:0;background:radial-gradient(ellipse 70% 60% at 65% 45%,rgba(135,206,235,0.12) 0%,transparent 65%),radial-gradient(ellipse 50% 40% at 20% 80%,rgba(255,182,193,0.10) 0%,transparent 60%);}
...
.hero-badge{display:inline-flex;align-items:center;gap:8px;background:rgba(135,206,235,0.1);border:1px solid rgba(135,206,235,0.3);padding:6px 16px;border-radius:100px;font-size:0.78rem;color:var(--cyan);font-weight:500;margin-bottom:32px;animation:fadeUp 0.6s ease both;}
...
.card:hover{border-color:rgba(135,206,235,0.25);transform:translateY(-4px);}
.card:hover::after{opacity:1;}
.icon{width:46px;height:46px;border-radius:12px;background:rgba(135,206,235,0.1);display:flex;align-items:center;justify-content:center;font-size:1.3rem;margin-bottom:18px;}
...
.pc.feat{background:linear-gradient(145deg,rgba(135,206,235,0.12),rgba(255,182,193,0.08));border-color:rgba(135,206,235,0.4);transform:scale(1.03);box-shadow:0 20px 60px rgba(135,206,235,0.18);}
...
.pbtn.p{background:var(--cyan);color:var(--navy);box-shadow:0 6px 20px rgba(135,206,235,0.3);}
...
.cta::before{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:700px;height:500px;background:radial-gradient(ellipse,rgba(135,206,235,0.12),transparent 70%);pointer-events:none;}
...
.accard:hover{border-color:rgba(135,206,235,0.25);transform:translateY(-4px);}
...
.section-label { color: #87ceeb; font-size: 12px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 12px; }
...
.demo-tab { padding: 10px 20px; border-radius: 24px; border: 1px solid rgba(135,206,235,0.25); background: rgba(135,206,235,0.05); color: #a0aec0; font-size: 13px; font-weight: 600; cursor: pointer; transition: all .2s; }
.demo-tab:hover { border-color: #87ceeb; color: #87ceeb; }
.demo-tab.active { background: rgba(135,206,235,0.15); border-color: #87ceeb; color: #87ceeb; }
...
.mock-logo-sm { color: #87ceeb; font-weight: 900; font-size: 16px; margin-bottom: 20px; padding: 0 4px; }
.mock-nav-item { padding: 7px 8px; border-radius: 6px; font-size: 11px; color: #4a5568; margin-bottom: 4px; cursor: default; }
.mock-nav-item.active-nav { background: rgba(135,206,235,0.1); color: #87ceeb; }
...
.mock-kpi-val.cyan { color: #87ceeb; }
...
.map-info { background: rgba(135,206,235,0.1); border: 1px solid rgba(135,206,235,0.3); border-radius: 20px; padding: 4px 10px; font-size: 11px; color: #87ceeb; }
...
.ia-cursor { animation: blink 1s infinite; color: #87ceeb; }
...
.rg-header { font-size: 14px; font-weight: 700; color: #87ceeb; margin-bottom: 14px; }
...
.rg-item.stop { background: rgba(135,206,235,0.05); border-left: 3px solid #87ceeb44; }
.rg-icon { font-size: 14px; flex-shrink: 0; }
.rg-loc { color: #e2e8f0; font-weight: 600; margin-bottom: 2px; }
.rg-detail { color: #718096; }
.rg-emergency { margin-top: 12px; padding: 8px 10px; background: rgba(135,206,235,0.08); border-radius: 8px; font-size: 11px; color: #87ceeb; font-weight: 600; }
...
.dtab2:hover{border-color:#87ceeb;color:#87ceeb}
.dtab2.dtab-on{background:rgba(135,206,235,0.12);border-color:#87ceeb;color:#87ceeb}
...
<div style="text-align:center;margin-bottom:40px">
  <div style="color:#87ceeb;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px" data-i18n="demo_lbl">Plataforma em ação</div>
...
<div style="border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);box-shadow:0 24px 64px rgba(0,0,0,0.6)">
  ...
  <div id="dp0" style="background:#0d1117;display:flex;min-height:360px">
    <div style="width:155px;background:#111827;padding:14px 10px;flex-shrink:0;border-right:1px solid rgba(255,255,255,0.06)">
      <div style="color:#87ceeb;font-weight:900;font-size:16px;margin-bottom:20px;padding:0 4px">FNI</div>
      <div style="padding:7px 8px;border-radius:6px;background:rgba(135,206,235,0.1);color:#87ceeb;font-size:11px;font-weight:600;margin-bottom:4px">📈 Dashboard</div>
    ...
    <div style="flex:1;padding:16px;overflow:hidden">
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">
        <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:12px 8px;text-align:center"><div style="font-size:20px;font-weight:800;color:#87ceeb;margin-bottom:4px">2.960</div><div style="font-size:9px;color:#4a5568">Postos ANP</div></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:12px">
          ...
          <div style="display:flex;flex-direction:column;gap:6px">
            <div style="display:flex;align-items:center;gap:8px;font-size:11px;color:#718096"><span style="width:22px">SP</span><div style="height:9px;border-radius:5px;background:#87ceeb;width:85%"></div><span>85%</span></div>
            <div style="display:flex;align-items:center;gap:8px;font-size:11px;color:#718096"><span style="width:22px">MG</span><div style="height:9px;border-radius:5px;background:rgba(135,206,235,0.8);width:72%"></div><span>72%</span></div>
            <div style="display:flex;align-items:center;gap:8px;font-size:11px;color:#718096"><span style="width:22px">RS</span><div style="height:9px;border-radius:5px;background:rgba(135,206,235,0.6);width:61%"></div><span>61%</span></div>
            <div style="display:flex;align-items:center;gap:8px;font-size:11px;color:#718096"><span style="width:22px">PR</span><div style="height:9px;border-radius:5px;background:rgba(135,206,235,0.4);width:54%"></div><span>54%</span></div>
            <div style="display:flex;align-items:center;gap:8px;font-size:11px;color:#718096"><span style="width:22px">BA</span><div style="height:9px;border-radius:5px;background:rgba(135,206,235,0.25);width:43%"></div><span>43%</span></div>
          </div>
        </div>
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:12px">
          ...
          <svg viewBox="0 0 180 80" style="width:100%;height:70px">
            <defs><linearGradient id="lg2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#87ceeb" stop-opacity=".25"/><stop offset="100%" stop-color="#87ceeb" stop-opacity="0"/></linearGradient></defs>
            <path d="M0,62 L30,58 L60,61 L90,50 L120,44 L150,40 L180,35" fill="none" stroke="#87ceeb" stroke-width="2" stroke-linejoin="round"/>
            ...
            <text x="148" y="33" font-size="9" fill="#87ceeb" font-weight="600">R$6,05</text>
          </svg>
        </div>
      </div>
    </div>
  ...
  <div id="dp1" style="display:none;background:#0d1117">
    <div style="background:#111827;padding:10px 14px;display:grid;grid-template-columns:1fr 1fr 80px;gap:8px;border-bottom:1px solid rgba(255,255,255,0.06)">
      ...
      <div style="background:#87ceeb;border-radius:6px;padding:7px 10px;font-size:12px;font-weight:700;color:#0d1117;text-align:center">Buscar</div>
    </div>
    <div style="position:relative;height:240px;overflow:hidden;background:#1a2744">
      <svg viewBox="0 0 600 240" style="width:100%;height:100%" preserveAspectRatio="xMidYMid slice">
        ...
        <path d="M70,200 C130,175 200,150 270,122 C340,94 420,70 500,48 C530,40 550,36 570,32" fill="none" stroke="#87ceeb" stroke-width="9" opacity="0.1" stroke-linecap="round"/>
        <path d="M70,200 C130,175 200,150 270,122 C340,94 420,70 500,48 C530,40 550,36 570,32" fill="none" stroke="#87ceeb" stroke-width="2.5" stroke-linecap="round" stroke-dasharray="1000" stroke-dashoffset="1000">
        ...
        <g opacity="0"><animate attributeName="opacity" from="0" to="1" dur=".3s" begin="0.9s" fill="freeze"/>
          <circle cx="180" cy="163" r="8" fill="#87ceeb"/>
          <circle cx="180" cy="163" r="16" fill="none" stroke="#87ceeb" stroke-width="1.5"><animate attributeName="r" from="8" to="24" dur="2s" repeatCount="indefinite"/><animate attributeName="opacity" from="0.4" to="0" dur="2s" repeatCount="indefinite"/></circle>
          <rect x="190" y="149" width="68" height="16" rx="8" fill="rgba(0,0,0,0.85)"/>
          <text x="224" y="161" text-anchor="middle" font-size="9" fill="#87ceeb" font-weight="700">R$5,66/L</text>
        </g>
        <g opacity="0"><animate attributeName="opacity" from="0" to="1" dur=".3s" begin="1.2s" fill="freeze"/>
          <circle cx="290" cy="117" r="8" fill="#87ceeb"/>
          <circle cx="290" cy="117" r="16" fill="none" stroke="#87ceeb" stroke-width="1.5"><animate attributeName="r" from="8" to="24" dur="2s" begin=".5s" repeatCount="indefinite"/><animate attributeName="opacity" from="0.4" to="0" dur="2s" begin=".5s" repeatCount="indefinite"/></circle>
          <rect x="300" y="103" width="60" height="16" rx="8" fill="rgba(0,0,0,0.85)"/>
          <text x="330" y="115" text-anchor="middle" font-size="9" fill="#87ceeb">R$5,90/L</text>
        </g>
      ...
      <div style="position:absolute;bottom:10px;left:12px;right:12px;display:flex;justify-content:space-between;align-items:flex-end;pointer-events:none">
        ...
        <div style="background:rgba(135,206,235,0.15);border:1px solid rgba(135,206,235,0.4);border-radius:20px;padding:5px 14px;font-size:11px;color:#87ceeb;font-weight:600">⛽ 12 postos · 408 km · 4h 20min</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:#111827">
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:12px"><span style="color:#ffd600;font-weight:800;width:24px;flex-shrink:0">1</span><span style="flex:1;color:#e2e8f0">Posto Dallas — Registro/SP</span><span style="color:#4a5568;font-size:10px;width:40px">2,1 km</span><span style="color:#87ceeb;font-weight:700;margin-right:6px">R$5,66/L</span><span style="background:rgba(72,187,120,0.15);color:#48bb78;padding:2px 8px;border-radius:10px;font-size:10px">-12% ANP</span></div>
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:12px"><span style="color:#a0aec0;font-weight:800;width:24px;flex-shrink:0">2</span><span style="flex:1;color:#e2e8f0">Auto Posto Kadoshi — Ponta Grossa/PR</span><span style="color:#4a5568;font-size:10px;width:40px">3,4 km</span><span style="color:#87ceeb;font-weight:700;margin-right:6px">R$5,90/L</span><span style="background:rgba(72,187,120,0.15);color:#48bb78;padding:2px 8px;border-radius:10px;font-size:10px">-8% ANP</span></div>
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;font-size:12px"><span style="color:#cd7f32;font-weight:800;width:24px;flex-shrink:0">3</span><span style="flex:1;color:#e2e8f0">Posto Muquém — Curitiba/PR</span><span style="color:#4a5568;font-size:10px;width:40px">1,8 km</span><span style="color:#87ceeb;font-weight:700;margin-right:6px">R$5,98/L</span><span style="background:rgba(72,187,120,0.15);color:#48bb78;padding:2px 8px;border-radius:10px;font-size:10px">-7% ANP</span></div>
    </div>
  </div>
  ...
  <div id="dp3" style="display:none;background:#0d1117;padding:16px;min-height:360px">
    <div style="font-size:15px;font-weight:700;color:#87ceeb;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.08)">Rotograma — SP → BH <span style="font-size:11px;color:#4a5568;font-weight:400;margin-left:8px">João Silva · ABC-1234</span></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      ...
      <div>
        ...
        <div style="padding:10px;border-radius:8px;border-left:3px solid rgba(135,206,235,0.5);background:rgba(135,206,235,0.05);margin-bottom:7px"><div style="font-size:12px;font-weight:600;color:#e2e8f0;margin-bottom:3px">Posto Ipiranga — km 210</div><div style="font-size:10px;color:#4a5568">Abastecimento · R$6,05/L · 24h</div></div>
        <div style="padding:10px;border-radius:8px;border-left:3px solid rgba(135,206,235,0.5);background:rgba(135,206,235,0.05);margin-bottom:7px"><div style="font-size:12px;font-weight:600;color:#e2e8f0;margin-bottom:3px">Restaurante Rotatoria — km 310</div><div style="font-size:10px;color:#4a5568">Alimentacao · Aberto 24h</div></div>
        <div style="padding:10px;border-radius:8px;border-left:3px solid rgba(135,206,235,0.5);background:rgba(135,206,235,0.05)"><div style="font-size:12px;font-weight:600;color:#e2e8f0;margin-bottom:3px">Hotel Rodovario BH — km 490</div><div style="font-size:10px;color:#4a5568">Pernoite seguro · (31) 3333-4444</div></div>
      </div>
      <div style="grid-column:1/-1;background:rgba(135,206,235,0.08);border:1px solid rgba(135,206,235,0.2);border-radius:8px;padding:10px 14px;font-size:12px;color:#87ceeb;font-weight:600;text-align:center">PRF 191 · SAMU 192 · Bombeiros 193 · PM 190 · ANTT 166</div>
    </div>
  </div>
  <div id="dp4" style="display:none;background:#0d1117;padding:16px;min-height:360px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.08)">
      <div style="font-size:14px;font-weight:700;color:#87ceeb">Frete #1042 — São Paulo/SP → Curitiba/PR</div>
      <span style="background:rgba(135,206,235,0.12);color:#87ceeb;font-size:10px;font-weight:700;padding:4px 10px;border-radius:12px;flex-shrink:0">Aberto a propostas</span>
    </div>
    <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:16px">
      <div>
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:14px;margin-bottom:10px">
          ...
          <div style="font-size:12px;color:#87ceeb;font-weight:600">💰 Valor ofertado: R$ 2.850</div>
        </div>
      ...
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:14px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
          <div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#87ceeb,#a8def0);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13px;color:#26303d;flex-shrink:0">JS</div>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:10px">
          ...
          <span style="background:rgba(135,206,235,0.1);color:#87ceeb;font-size:10px;padding:3px 8px;border-radius:10px">🏷️ Pontual</span>
          <span style="background:rgba(135,206,235,0.1);color:#87ceeb;font-size:10px;padding:3px 8px;border-radius:10px">🏷️ Cuidado com a carga</span>
        </div>
  ...
  <div id="dp5" style="display:none;background:#0d1117;padding:16px;min-height:360px">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid rgba(255,255,255,0.08)">
      <div style="width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,#87ceeb,#a8def0);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:17px;color:#26303d;flex-shrink:0">JS</div>
      <div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">
      ...
      <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:10px;text-align:center"><div style="font-size:16px;font-weight:800;color:#87ceeb">184</div><div style="font-size:9px;color:#4a5568">Fretes concluídos</div></div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px">
      <span style="background:rgba(135,206,235,0.1);color:#87ceeb;font-size:11px;padding:5px 12px;border-radius:14px">🏷️ Pontual (14)</span>
      <span style="background:rgba(135,206,235,0.1);color:#87ceeb;font-size:11px;padding:5px 12px;border-radius:14px">🏷️ Cuidado com a carga (11)</span>
      <span style="background:rgba(135,206,235,0.1);color:#87ceeb;font-size:11px;padding:5px 12px;border-radius:14px">🏷️ Comunicativo (7)</span>
    </div>
  ...
  <div id="dp6" style="display:none;background:#0d1117;padding:16px;min-height:360px">
    <div style="font-size:14px;font-weight:700;color:#87ceeb;margin-bottom:14px">🎮 Missões da Rede — Julho/2026</div>
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:12px 14px">
        ...
        <div style="height:8px;border-radius:4px;background:rgba(255,255,255,0.06);overflow:hidden"><div style="height:100%;width:70%;background:#87ceeb;border-radius:4px"></div></div>
    ...
    <div style="margin-top:14px;background:rgba(135,206,235,0.06);border:1px solid rgba(135,206,235,0.2);border-radius:8px;padding:10px 14px;font-size:11px;color:#87ceeb">🏪 Parcerias locais desbloqueadas: 15% off na Borracharia Central e no Restaurante do Caminhoneiro</div>
  </div>
  <div id="dp7" style="display:none;background:#0d1117;padding:16px;min-height:360px">
    <div style="font-size:14px;font-weight:700;color:#87ceeb;margin-bottom:14px">🛡️ Alertas Antifraude — últimas 24h</div>
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border-radius:8px;background:rgba(135,206,235,0.05);border-left:3px solid #87ceeb44">
        <div style="font-size:15px">✅</div>
    ...
    <div style="margin-top:16px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
      ...
      <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:10px;text-align:center"><div style="font-size:16px;font-weight:800;color:#87ceeb">100%</div><div style="font-size:9px;color:#4a5568">Abastecimentos monitorados</div></div>
    </div>
  </div>
  <div id="dp8" style="display:none;background:#0d1117;padding:24px 16px;min-height:360px">
    <div style="display:flex;justify-content:center">
      <div style="width:230px;background:#111827;border:1px solid rgba(255,255,255,0.1);border-radius:22px;padding:14px;box-shadow:0 20px 50px rgba(0,0,0,0.5)">
        ...
        <div style="background:linear-gradient(135deg,#1a1040,#0d1a3a);border:1px solid rgba(135,206,235,0.25);border-radius:12px;padding:12px;margin-bottom:10px;text-align:center">
        ...
        <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:8px 10px;margin-bottom:10px">
          ...
          <div style="height:6px;border-radius:3px;background:rgba(255,255,255,0.08)"><div style="height:100%;width:70%;background:#87ceeb;border-radius:3px"></div></div>
        </div>
...
<section class="section" id="acesso">
  ...
  <div class="acgrid">
    <div class="accard">
      <svg viewBox="0 0 200 200" role="img" aria-label="Painel web"><rect x="14" y="30" width="172" height="120" rx="8" fill="none" stroke="#a8def0" stroke-width="3"/><rect x="14" y="30" width="172" height="18" rx="8" fill="#a8def0" fill-opacity="0.15"/><circle cx="26" cy="39" r="3" fill="#a8def0"/><circle cx="36" cy="39" r="3" fill="#a8def0" fill-opacity="0.5"/><circle cx="46" cy="39" r="3" fill="#a8def0" fill-opacity="0.3"/><rect x="28" y="66" width="60" height="34" rx="4" fill="#a8def0" fill-opacity="0.12"/><rect x="96" y="66" width="60" height="34" rx="4" fill="#a8def0" fill-opacity="0.12"/><rect x="28" y="108" width="128" height="30" rx="4" fill="#a8def0" fill-opacity="0.08"/><rect x="70" y="160" width="60" height="10" rx="5" fill="#a8def0" fill-opacity="0.3"/></svg>
...
<footer>
  ...
  <div class="fcopy" style="line-height:1.9">
    ...
    <a href="mailto:contato@fxgestaodefrotasonline.com" style="color:#a8def0">contato@fxgestaodefrotasonline.com</a>
    &nbsp;&middot;&nbsp; Brasil &nbsp;&middot;&nbsp;
    <a href="/privacidade" style="color:#a8def0">LGPD</a>
  </div>
</footer>
